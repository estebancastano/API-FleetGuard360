...
public class ReportController {
    ...
    /*public void exportAllReportsToPdf(HttpServletResponse response) throws IOException {
        List<FleetReport> reports = reportRepository.findAll();
        if (reports.isEmpty()) {
            response.setStatus(HttpServletResponse.SC_NO_CONTENT);
            return;
        }
        exportService.exportAllReportsToPdf(reports, response);
    }*/

    @GetMapping("/export/all/pdf")
    @PreAuthorize("hasRole('ADMIN') or hasRole('OPERATOR') or hasRole('VIEWER')")
    @Operation(summary = "Exportar todos los reportes a PDF", description = "Genera un archivo PDF con todos los reportes del sistema")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "PDF generado exitosamente",
                    content = @Content(mediaType = "application/pdf")),
